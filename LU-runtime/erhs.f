...
!$OMP&  SHARED(dz5,dz4,dz3,dz2,tz1,dz1,tz3,tz2,dy5,dy4,dy3,dy2,ty1,dy1,
!$OMP& ty3,ty2,dssp,dx5,dx4,dx3,dx2,tx1,dx1,tx3,tx2,nx,ny,nz)
      do k = 1, nz
...
!$OMP DO SCHEDULE(RUNTIME)
      do j = 2, ny-1
         do i = 2, nx-1
            do k = 1, nz
               rtmp(1,k) = rsd(1,i,j,k)
               rtmp(2,k) = rsd(2,i,j,k)
               rtmp(3,k) = rsd(3,i,j,k)
               rtmp(4,k) = rsd(4,i,j,k)
               rtmp(5,k) = rsd(5,i,j,k)
            end do

            do k = 1, nz
               flux(1,k) = rtmp(4,k)
               u41 = rtmp(4,k) / rtmp(1,k)
               q = 0.50d+00 * (  rtmp(2,k) * rtmp(2,k)
     >                         + rtmp(3,k) * rtmp(3,k)
     >                         + rtmp(4,k) * rtmp(4,k) )
     >                      / rtmp(1,k)
               flux(2,k) = rtmp(2,k) * u41 
               flux(3,k) = rtmp(3,k) * u41 
               flux(4,k) = rtmp(4,k) * u41 + c2 * 
     >                         ( rtmp(5,k) - q )
               flux(5,k) = ( c1 * rtmp(5,k) - c2 * q ) * u41
            end do

            do k = 2, nz - 1
               do m = 1, 5
                  utmp(m,k) =  frct(m,i,j,k)
     >                  - tz2 * ( flux(m,k+1) - flux(m,k-1) )
               end do
            end do

            do k = 2, nz
               tmp = 1.0d+00 / rtmp(1,k)
               ...
               tmp = 1.0d+00 / rtmp(1,k-1)

               u21km1 = tmp * rtmp(2,k-1)
               u31km1 = tmp * rtmp(3,k-1)
               u41km1 = tmp * rtmp(4,k-1)
               u51km1 = tmp * rtmp(5,k-1)
...
c---------------------------------------------------------------------
            do m = 1, 5
               frct(m,i,j,2) = utmp(m,2)
     >           - dssp * ( + 5.0d+00 * rtmp(m,2)
     >                       - 4.0d+00 * rtmp(m,3)
     >                       +           rtmp(m,4) )
               frct(m,i,j,3) = utmp(m,3)
     >           - dssp * (- 4.0d+00 * rtmp(m,2)
     >                      + 6.0d+00 * rtmp(m,3)
     >                      - 4.0d+00 * rtmp(m,4)
     >                      +           rtmp(m,5) )
            end do
     ...
     >                        +           rtmp(m,k+2) )
            ...
            do m = 1, 5
               frct(m,i,j,nz-2) = utmp(m,nz-2)
     >           - dssp * (            rtmp(m,nz-4)
     >                      - 4.0d+00 * rtmp(m,nz-3)
     >                      + 6.0d+00 * rtmp(m,nz-2)
     >                      - 4.0d+00 * rtmp(m,nz-1)  )
               frct(m,i,j,nz-1) = utmp(m,nz-1)
     >           - dssp * (             rtmp(m,nz-3)
     >                       - 4.0d+00 * rtmp(m,nz-2)
     >                       + 5.0d+00 * rtmp(m,nz-1)  )
            end do
